...
\begin{itemize}
    \item \textbf{Sistematização do Monitoramento e Curadoria:} Embora a transição e a validação do dado sejam automatizadas, a etapa de monitoramento da execução e a curadoria de exceções (como a inclusão de novos produtos no dicionário) dependem da ação do manutentor. Sistematizar o ambiente de curadoria da informação é um passo futuro importante para garantir que a etapa humana, naturalmente suscetível a falhas ou atrasos, não comprometa a esteira de entrega final dos dados.
    \item \textbf{Implementação de Validação Numérica e Estatística:} O \textit{pipeline} atual garante a integridade estrutural e a reconciliação matemática. Como evolução de maturidade do projeto, sugere-se a adição de uma malha de validação comportamental baseada em análise de série histórica. O sistema passaria a avaliar a curva esperada de uma determinada variável e, caso os dados numéricos extraídos fujam expressivamente do desvio padrão (caracterizando um \textit{outlier}), o processamento acenderia um alerta prévio ao manutentor para averiguação, adicionando uma rigorosa camada de validação semântica ao fluxo.
\end{itemize}
