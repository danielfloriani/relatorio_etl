...
O fluxo se inicia quando o desenvolvedor responsável pelo monitoramento recebe o documento da fonte (via e-mail ou portais do mercado). O arquivo é então salvo de maneira padronizada no diretório de entrada inicial do \textit{pipeline} (denominado \texttt{01\_RAW}). Após o armazenamento, o usuário aciona a execução do processamento através de um \textit{bot} (robô virtual de automação) no aplicativo de mensagens \textit{Telegram}, o qual atua como o gatilho externo. Este \textit{bot} envia um comando (via \textit{webhook}, um mecanismo de comunicação em tempo real, ou requisição de API - interface de comunicação entre sistemas) que inicia a rotina de execução do repositório no \textit{GitHub} (plataforma em nuvem de hospedagem e versionamento de código-fonte), transferindo o processamento pesado para o ambiente em nuvem da aplicação.

\section{Fluxo e Evolução Estrutural dos Arquivos}

A \autoref{fig_arquitetura} apresenta o diagrama da Arquitetura \textit{Medallion} implementada, ilustrando a transição dos dados desde a sua captura bruta até a disponibilização para os analistas de mercado. Uma diretriz central desta arquitetura foi o isolamento contínuo das fontes de dados durante as etapas primárias de limpeza e validação, a fim de evitar que anomalias em um relatório comprometessem a ingestão do outro.
...
Para operacionalizar esse fluxo lógico, a \autoref{fig_fluxo} detalha visualmente as etapas de processamento executadas pelos algoritmos orquestradores da esteira.

\begin{figure}[htb]
    \caption{\label{fig_fluxo}Fluxo de Processamento de \texttt{main\_source\_X.py} ou \texttt{main\_source\_Y.py}}
    \begin{center}
...
A transição e consolidação da informação ocorrem de maneira gradativa através das camadas detalhadas a seguir:
...
\begin{itemize}
    ...
    \item \textbf{Camada \textit{Bronze} (L0 Original e Revisada):} Após o acionamento do \textit{parser} (algoritmo de extração de texto), o dado é convertido do formato PDF para arquivos tabulares no formato .CSV (\textit{Comma-Separated Values}), preservando a granularidade original. Assim, na camada L0, cada CSV gerado continua representando os dados de uma única fonte para uma referência temporal específica. A fragmentação da L0 permite a tolerância a falhas na extração do texto antes de avançar para as validações estruturais.
    ...
    \item \textbf{Camada \textit{Gold} (L2):} É na penúltima camada de processamento que ocorre a consolidação global do \textit{pipeline}. Os dados validados das diferentes origens (Fonte X e Fonte Y) convergem para formar um único documento mestre de \textit{data warehouse} (armazém central de dados) \cite{kimball13}. A granularidade torna-se abrangente e multiorigem. 
    
    \item \textbf{Camada de Apresentação (\textit{View}):} Atuando como a interface final entre a engenharia de dados e a equipe de inteligência de mercado, esta camada lógica consome os dados consolidados da L2. Sua principal função na arquitetura é adequar a granularidade técnica para uma visão puramente de negócios: metadados exclusivos de auditoria do sistema são filtrados e instâncias redundantes (geradas por republicações de períodos anteriores) são ocultadas. O arquivo resultante finaliza o fluxo representando uma visão definitiva e cronologicamente atualizada, otimizada para o consumo direto pelas ferramentas de \textit{Business Intelligence}.
\end{itemize}